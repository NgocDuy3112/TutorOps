...
export function DashboardPage() {
  ...
  function startEditSession(session: TeachingSession, studentName: string) {
    ...
    setSelectedStudent({ id: session.studentId, name: studentName, defaultPriceVnd: session.priceVnd });
  }
        ...
        <Card className="overflow-hidden rounded-3xl border-slate-200 shadow-sm shadow-slate-200/70">
          <CardHeader className="space-y-3 border-b border-slate-100 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button type="button" size="icon" className="min-h-11 rounded-full" onClick={startCreateSession} aria-label="Thêm buổi dạy">
                <Plus size={18} />
              </Button>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-12 rounded-2xl border p-1 text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary sm:min-h-14 ${
                        selected ? "border-primary bg-primary text-primary-foreground shadow-md shadow-violet-200" : "border-transparent hover:bg-slate-100"
                    ...
                    >
                      ...
                      <span className="mt-1 flex justify-center gap-1">
                        ...
                        {dayAssignments.slice(0, 2).map((assignment) => (
                          <span key={assignment.id} className={`block size-1.5 rounded-full ${deadlineColor(assignment.dueAt!)}`} />
                        ))}
                      </span>
                    </button>
                  );
          ...
          <div className="grid gap-2">
            {students.map((student) => (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">{formatVnd(student.defaultPriceVnd)} / buổi</span>
              </button>
            ))}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Tổng tiền buổi dạy: {formatVnd(selectedTotal)}</DialogDescription>
        </DialogHeader>
        ...
        <div className="max-h-[65dvh] space-y-3 overflow-y-auto pr-1">
          {groups.length === 0 ? (
            ...
            groups.map((group) => {
              ...
              return (
                <section key={group.studentId} className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm shadow-slate-100">
                  <div className="mb-4 flex items-start justify-between gap-3">
                    ...
                    {groupTotal > 0 && <p className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-sm font-bold">{formatVnd(groupTotal)}</p>}
                  </div>

                  <div className="space-y-2">
                    {group.sessions.map((session) => (
                      <article key={session.id} className="rounded-2xl bg-violet-50 p-3">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <div className="flex shrink-0 items-center gap-2">
                            <p className="text-sm font-bold">{formatVnd(session.priceVnd)}</p>
                            <Button type="button" variant="outline" size="sm" className="min-h-11" onClick={() => onEdit(session, group.studentName)}>
                              <Pencil size={15} />
                              Sửa
                            </Button>
                          </div>
                        </div>
                        {session.note && <p className="mt-3 rounded-xl bg-white/70 p-3 text-sm text-slate-700">{session.note}</p>}
                      </article>
                    ))}

                    {group.assignments.map((assignment) => (
                      <article key={assignment.id} className="rounded-2xl bg-slate-50 p-3">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex min-w-0 gap-3">
                            <span className={`grid size-9 shrink-0 place-items-center rounded-full text-white ${deadlineColor(assignment.dueAt!)}`}>
                              <BookOpenCheck size={17} />
                            </span>
                            <div className="min-w-0">
                              <h5 className="truncate text-sm font-semibold">{assignment.title}</h5>
                          ...
                          <p className="shrink-0 rounded-full bg-white px-2 py-1 text-xs text-muted-foreground">Bài tập</p>
                        </div>
        ...
        <Button type="button" className="min-h-11 w-full rounded-2xl" onClick={onCreate}>
          <Plus size={16} />
          Ghi nhận buổi dạy
        </Button>
      </DialogContent>
    ...
    <div className="animate-pulse">
      <div className="grid grid-cols-7 gap-1 text-center">
        ...
        {Array.from({ length: 42 }, (_, index) => (
          <div key={`day-${index}`} className="min-h-12 rounded-xl bg-slate-100 sm:min-h-14" />
        ))}
      </div>
    </div>
  );
