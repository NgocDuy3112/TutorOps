...
export function StudentProfilePage({ studentId }: { studentId: string }) {
  ...
  async function load() {
    ...
    try {
      ...
      setPaymentData(await paymentsResponse.json());
    } catch (requestError) {
      setError(
  ...
  return (
    <MobileShell>
      ...
      <main className="mx-auto max-w-3xl px-4 py-5">
        ...
        {tab === "overview" && (
          <section className="space-y-4 pt-5">
            <Button
              ...
              className="min-h-12 w-full justify-start rounded-xl px-4 text-left"
            >
            ...
            <Card>
              <CardContent className="p-4">
                ...
                <p className="mt-3 text-2xl font-bold text-amber-600">
                  {formatVnd(totalSessions)}
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
                  Công nợ hiện tại: {formatVnd(paymentData?.balance ?? 0)}
                </p>
              </CardContent>
            </Card>
          </section>
        ...
        {tab === "assignments" && (
          <AssignmentsTab assignments={studentAssignments} />
        )}
        {tab === "tuition" && (
          <TuitionTab
            data={paymentData}
            onRecord={() => setShowPaymentForm(true)}
          />
        )}
      </main>
      {showSessionForm && (
      ...
      <PaymentDialog
        student={showPaymentForm ? student : null}
        balance={paymentData?.balance ?? 0}
        onOpenChange={(open) => !open && setShowPaymentForm(false)}
        onSaved={() => {
...
function ProfileTabs({
  ...
  return (
    <div
      ...
      className="grid grid-cols-2 gap-2 rounded-2xl border border-slate-200 bg-slate-100 p-2 sm:grid-cols-4"
    >
      {tabs.map((item) => {
        ...
        sessions.map((item) => (
          <Card key={item.id}>
            <CardContent className="flex justify-between gap-3 p-4 text-sm">
              <div>
                <strong>
                  {new Date(item.taughtAt).toLocaleString("vi-VN")}
                </strong>
                <p className="mt-1 text-muted-foreground">
                  {item.note || "Buổi dạy"}
                </p>
              </div>
              <strong>{formatVnd(item.priceVnd)}</strong>
            </CardContent>
          </Card>
        ))
      )}
  ...
  return (
    <section className="space-y-3 pt-4">
      {assignments.length === 0 ? (
        <Empty text="Chưa giao bài tập cho học sinh này." />
      ) : (
        assignments.map((item) => {
          const status = item.students.find((student) => student.id)?.status;
          return (
            <Card key={item.id}>
              <CardContent className="p-4">
                <div className="flex justify-between gap-3">
                  ...
                  <span className="text-sm font-medium text-primary">
                    {status === "submitted" ? "Đã nộp" : "Chờ nộp"}
                  </span>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">
                  {item.dueAt
    ...
    <section className="space-y-4 pt-4">
      <Card className="border-slate-200 bg-white text-slate-900 shadow-sm">
        <CardContent className="grid gap-3 p-5 sm:grid-cols-3">
          <div>
            <p className="text-sm text-muted-foreground">Cần thu</p>
            <strong>{formatVnd(data?.totalDue ?? 0)}</strong>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Đã thu</p>
            <strong>{formatVnd(data?.totalPaid ?? 0)}</strong>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Còn lại</p>
            <strong>{formatVnd(data?.balance ?? 0)}</strong>
          </div>
        </CardContent>
      </Card>
      <Button type="button" className="min-h-11 w-full" onClick={onRecord}>
        Ghi nhận thanh toán
      </Button>
